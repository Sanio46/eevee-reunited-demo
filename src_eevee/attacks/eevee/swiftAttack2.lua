...
---@field Parent Entity
---@field ActiveWeapons SwiftWeapon[]
...
function swift:GetInstanceCooldown(player)
	...
	return fireDelay + math.abs(fireDelay)
end
...
function swift:GetStartingPos(swiftData)
	local angle = (360 / swiftData.NumWeaponsToSpawn) * swiftData.NumWeaponsSpawned
	return Vector.FromAngle(angle):Resized(swift:SwiftOrbitDistance(swiftData.Player))
end

function swift:WeaponToInstanceWeapon(swiftWeapon, weapon)
	local isCorrectWeapon = false
	if tostring(GetPtrHash(swiftWeapon)) == tostring(GetPtrHash(weapon)) then
...
function swift:FireSwift(swiftData, player, parent)
	...
	---@type SwiftWeapon
	local swiftWeapon = swiftData.ActiveWeapons[tostring(GetPtrHash(swiftTear))]
...
function swift:RemoveWeaponOnDeath(weapon)
	for _, swiftData in ipairs(swiftInstances) do
		local swiftWeapon = swiftData.ActiveWeapons[tostring(GetPtrHash(weapon))]
...
function swift:InitInstance(player)
	...
	if data.TimeTillNextInstance then
		if data.TimeTillNextInstance > 0 then
			data.TimeTillNextInstance = data.TimeTillNextInstance - 1
		else
...
function swift:CreateSwiftInstance(player, parent, customData)
	...
	for variableName, value in pairs(swiftInstanceData) do
		instance[variableName] = customData ~= nil and customData[variableName] or value
	end
...
function swift:InitSwiftWeapon(swiftData, weapon)
	...
	if swiftData.ActiveWeapons[ptrHashWeapon] == nil then
		swiftData.ActiveWeapons[ptrHashWeapon] = {}
		local swiftWeapon = swiftData.ActiveWeapons[ptrHashWeapon]
		for variableName, value in pairs(SwiftWeaponData) do
			swiftWeapon[variableName] = value
		end
		swiftWeapon.WeaponEntity = weapon
		swiftWeapon.WeaponIndex = swiftData.NumWeaponsSpawned
	end
end
...
function swift:InitInstanceValues(swiftData)
	...
	swiftData.TotalDuration = swiftData.TotalDuration ~= 0 and swiftData.TotalDuration or totalDuration + 0.5
	swiftData.DurationTimer = swiftData.DurationTimer ~= 0 and swiftData.DurationTimer or totalDuration + 0.5
	swiftData.WeaponSpawnTimer = swiftData.WeaponSpawnTimer ~= 0 and swiftData.WeaponSpawnTimer or fireDelay
end
...
function swift:SpawnNextWeapon(swiftData)
	...
	elseif swiftData.NumWeaponsSpawned < swiftData.NumWeaponsToSpawn then
		swiftData.NumWeaponsSpawned = swiftData.NumWeaponsSpawned + 0.5
...
function swift:OnPostPlayerUpdate(player)
	...
	for index, swiftData in ipairs(swiftInstances) do
		...
		if swiftData.CanFire then return end
...
function swift:OnPostTearUpdate(tear)
	for _, swiftData in ipairs(swiftInstances) do
		---@type SwiftWeapon
		local swiftWeapon = swiftData.ActiveWeapons[tostring(GetPtrHash(tear))]
		if swiftWeapon ~= nil then
