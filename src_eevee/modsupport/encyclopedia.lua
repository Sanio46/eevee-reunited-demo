...
local CharactersWiki = {
	PlayerEevee = {
		...
		{ --Traits
			...
			{ str = "While holding the fire buttons, stars will be summoned and continuously orbit around Eevee. After either letting go of the fire keys or after after 5 stars have been summoned, they will shoot forward in the direction you're firing." },
			{ str = "Summoned stars inherintly have spectral and homing." },
			{ str = "In addition to homing, stars can auto-aim onto enemies. This will only trigger if an enemy is within 30 degrees of the direction the star will fire, and will always choose the closest enemy." },
			{ str = "Eevee will have a cooldown before being able to use Swift again, determined by how long you charged up Swift. Significantly less if fired early on, and slightly longer if the full amount of stars are shot." },
		...
		{ -- Notes
			...
			{ str = "Eevee's x0.8 damage multiplier and having a cooldown to use Swift makes it difficult to face enemies up-front. Take advantage of Swift's range, spectral, and homing by keeping your distance as much as possible." },
			{ str = "Tapping the fire key will shoot a single star in the direction you're facing at a cooldown akin to firing like any other character. Use this to focus your firing if needed, such as fighting back Mega Troll Bombs." },
			...
			{ str = "Swift stars can carry over between rooms, so its possible to charge up a shot before it fires and moving into the next room." },
			{ str = "Early on, don't risk anything that could potentially give a damage or tears down. Avoid things like pills and Experimental Treatment in early floors unless you're feeling lucky in the earlier floors." },
		},
		{ -- Synergies
			...
			{ str = "Brimstone: Stars are replaced by Brimstone orbs. The orbs do continuous damage to anything that touches it, determined by the player's damage. Fires off Brimstone lasers when shitted. The rate that these orbs appear are determined are equal to your firerate x 1.2." },
			{ str = "Brimstone + Mom's Knife: When knives are fired, they will periodically glow red for one second, after which they will shoot out a Brimstone laser and propel themselves at twice their original velocity in the opposite direction" },
			{ str = "Dr. Fetus: Stars are replaced by bombs. These bombs will not start their detonation countdown until Swift fires them off." },
			{ str = "Eye Sore: For each star fired, it has a random chance to fire 1-3 additional stars in random directions. The direction is not influenced by Swift's auto-aim." },
			{ str = "Fire Mind / Ipecac: Stars orbit further away from the player, out of explosion radius." },
			{ str = "Loki's Horns: For each star fired, it has a random chance to fire additional stars in the remaining 3 cardinal directions. The direction is not influenced by Swift's auto-aim." },
			{ str = "Lost Contact: Stars orbit much closer to the player." },
			{ str = "Mom's Eye: For each star fired, it has a random chance to fire an additional star in the opposite direction. The direction is not influenced by Swift's auto-aim." },
			{ str = "Mom's Knife: Stars are replaced by floating knives that deal the player's damage. They have a minimum lifetime of 2 seconds before the player's range stat can take effect, and all fire simultaneously. The rate that these knives appear are determined are equal to your firerate x 1.3." },
			{ str = "Mom's Knife + Tech X: Tech X rings surround each knife, having a small radius and deals 25% of the player's damage." },
			{ str = "Monstro's Lung: Acts as a multi-tear modifier, having 5 additional stars orbit every initially spawned star, with their distance from the star being randomized." },
			{ str = "Neptunus: Swift automatically starts on its own without input required, and remains orbiting until you fire. Swift can generate up to two rings" },
			{ str = "Spirit Sword: Swift stars quickly spawn around Eevee during a spin attack, launching forward when the swing completes. However, the player can no longer fire their beam projectiles." },
			{ str = "Tech X: Stars are replaced by laser orbs. When fired, releases a laser ring. The ring scales in radius and damage equal to how long Swift has been charged, going from 25% to 100% damage at the beginning and end of Swift's charge respecively. The rate that these knives appear are determined are equal to your firerate x 1.5." },
			{ str = "Technology: Stars are replaced by technology orbs. The orbs do continuous damage to anything that touches it, determined by the player's damage. Once Swift fires, all surrounding orbs simultaneously fire a Technology laser in the direction the player fired them in." },
			{ str = "Technology 2: Constant firing technology lasers are attached to each star, dealing 20% of the player's damage and disappearing after the star has been fired." },
			{ str = "The Ludovico Technique: Replaces the single large tear with a spinning ring of Swift stars." },
			{ str = "Tiny Planet: Stars will continue to orbit the player much longer than normal, increasing their distance from the player over time while not affecting cooldown, allowing Swift to be fired again while stars are still orbiting. Stars also fire as normal, ignoring the usual Tiny Planet orbit." },
		},
		{ -- Interactions
			...
			{ str = "Anti-Gravity: The entire Swift star ring stays in place so long as you're still firing, rotating in place and firing as normal when fire keys are let go or after a set duration" },
			...
			{ str = "Evil Eye: Stars have a chance to be replaced by an Evil Eye, functioning as normal like the other stars." },
			...
			{ str = "Kidney Stone: Swift won't stop firing when Eevee's head turns red. The fired kidney stone still fires from the mouth. Once Kidney Stone is active, firerates can be high enough to have instances of Swift overlap one another." },
			...
			{ str = "Pop!: Orbiting stars can hit stars that are still floating, resulting in a better chance of more stars remaining on screen." },
			...
			{ str = "The Wiz: When a star is fired, it will split into two and shoot out at opposing 45 degree angles. This additional shot is influenced by Swift's auto-aim, unlike Loki's Horns or Mom's Eye" },
